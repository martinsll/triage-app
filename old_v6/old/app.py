...
def api_start():
    ...
    set_label      = data.get("set", "A")
    groups         = [int(g) for g in data.get("groups", [1, 2, 3])]
    language       = data.get("language", "en")

    sess = init_session_data(participant_id, set_label, groups, language)
    save_session(sess)

    session["session_id"] = sess["session_id"]
    session["set"]        = set_label
